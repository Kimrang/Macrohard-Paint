		...
		if(button != 0)
		{
			...
			else
			{
				...
				if((brushcolor==BLINK) || (brushcolor==BLACK))
					brushcolor = (rand() % 10)+1;
				setcolor(brushcolor);
				setfillstyle(SOLID_FILL, brushcolor);
				circle(getmaxx()-30, getmaxy()-15, 8);
				floodfill(getmaxx()-35, getmaxy()-20, brushcolor);
			}
		}
	}
	getch();
